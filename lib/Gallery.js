...
export default class App extends Component<Props> {
  ...
  componentDidUpdate (prevProps) {
    ...
    console.log('componentDidUpdate (prevProps, prevState) ', prevProps.imageIndex, this.props.imageIndex)
  ...
  renderItem = ({ item, index }) => (
    <TouchableOpacity onPress={() => this.handleIndex(index)}>
      <Image style={{ width: 75, height: 75 }} source={item.source} resizeMode='cover' />
    </TouchableOpacity>
    )

  handleIndex = imageIndex => {
    if (this.props.imageIndex === imageIndex) return
    console.log('handleIndex', imageIndex)
  ...
  render () {
    const { images, isImageViewVisible, imageIndex, onClose, setIndex } = this.props
