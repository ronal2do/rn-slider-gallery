...
const minus = Platform.OS === 'android' ? 105 : 80
...
export default class ImageView extends Component<PropsType, StateType> {
  ...
  componentWillReceiveProps(nextProps: PropsType, prevState) {
    const { images, imageIndex, isVisible } = this.state;

    const nextTick = new Promise(resolve => setTimeout(resolve, 0));
    nextTick.then(() => {
      this.listRef.scrollToIndex({
        index: nextProps.imageIndex,
        animated: true,
      });
    });
    ...
    if (
      ...
      if (
        ...
        this.getInitialTranslate(nextProps.imageIndex);
  ...
  onGestureRelease(event: NativeEventType, gestureState: GestureState) {
    if (typeof this.props.setIndex === 'function') {
      this.props.setIndex(this.state.imageIndex);
    }
    if (this.glideAlwaysTimer) {
      clearTimeout(this.glideAlwaysTimer);
    }
  ...
  getImageStyle(
    ...
    return { width, height, transform, top: 0 };
  }
